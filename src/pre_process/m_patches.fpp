...
contains
        ...
        elseif (n > 0) then
            ...
            do i = 1, num_ibs
                ...
                elseif (patch_ib(i)%geometry == 5) then
                    call s_model(i, ib_markers_sf, q_prim_vf, ib, levelset, levelset_norm)
                ! Seth Brolliar addition, triangle ib
                elseif (patch_ib(i)%geometry == 6) then
                    call s_triangle(i, ib_markers_sf, q_prim_vf, ib)
    ...
    !> The triangular patch is a 2D geometry primarily meant for
        ...
        !!              the middle back of the triangle. From this point the x
        !!              distance will move left, and the y distance will move up and down,
        !!              assuming the triangle is a 2 right triangles
        !! @param patch_id is the patch identifier
        !! @param patch_id_fp Array to track patch ids
        !! @param q_prim_vf Array of primitive variables
        !! @param ib True if this patch is an immersed boundary
    subroutine s_triangle(patch_id, patch_id_fp, q_prim_vf, ib_flag)
       ...
       ! @:Hardcoded2DVariables()
        ...
        if (present(ib_flag)) then
            ...
            x_corner = patch_icpp(patch_id)%x_centroid
            y_corner = patch_icpp(patch_id)%y_centroid
            length_x = patch_icpp(patch_id)%length_x
    ...
    subroutine s_model(patch_id, patch_id_fp, q_prim_vf, ib_flag, STL_levelset, STL_levelset_norm)
        ...
        if (present(ib_flag) .and. proc_rank == 0) then
