...
contains
    ...
    impure subroutine s_compute_image_points(ghost_points_in)
        ...
        $:GPU_PARALLEL_LOOP(private='[q, gp, i, j, k, physical_loc, patch_id, dist, norm, dim, bound, dir, index, temp_loc, s_cc, &
                            & local_capture]', copy='[bounds_error, bounds_error_capture_count, bounds_error_patch_id, &
        ...
        if (bounds_error) then
            if (bounds_error_capture_count >= 1 .and. bounds_error_dim >= 1 .and. bounds_error_dim <= num_dims) then
                ...
                print *, "Image point is located approximately ", (real(bounds_error_loc(dim), &
                    & wp) - bounds_error_ip_loc(dim))/(s_cc(1) - s_cc(0)), " grid cells away"
